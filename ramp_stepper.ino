...
void rampUp() {
  ...
  while(counter<5){
    ...
    delay(dt*freq/490.2);
    analogWrite(step_pin,0);
    delay(dt*(1-freq/490.2));
  }
  setPwmFrequency(step_pin, 1);
  while(millis() - initial_time < 0.5*ratio*tau || freq < max_freq){    
    counter++;
    freq = counter*.01*max_freq;
    analogWrite(step_pin, 128);
    delay(dt*freq/31372.55);
    analogWrite(step_pin,0);
    delay(dt*(1-freq/31372.55));   
  }
}
...
void rampDown() {
  ...
  digitalWrite(dir_pin, 255);
  double initial_time = millis();  
  while(millis() - initial_time < 0.5*ratio*tau || freq >= .01*max_freq){    
    counter++;
    freq = max_freq - counter*.01*max_freq;
    analogWrite(step_pin, 128);
    delay(dt*freq/31372.55);
    analogWrite(step_pin,0);
    delay(dt*(1-freq/31372.55));
